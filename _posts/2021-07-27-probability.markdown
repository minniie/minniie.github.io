...
&emsp;▹ event 1 = 1, event 2 = 0<br>
▸ event space: event 의 집합<br>
&emsp;▹ event space = {1,0}<br>
&emsp;▹ ~ state space<br>
▸ random variable(확률변수): 자연에서 일어나는 것들을 수학적으로 설명하기 위한 수단<br>
&emsp;▹ variable: 확률변수는 각 사건을 값으로 가진다<br>
&emsp;▹ function: 각 사건에 그 사건이 발생할 확률이 매핑되어 있다<br>
&emsp;▹ P(X=x): X is random variable, x is an event<br>
&emsp;▹ discrete random variable(이산확률변수): event 의 type 이 countable 한 경우<br>
&emsp;&emsp;▸ countable: finite (동전, 주사위) or countably infinite (integers)<br>
&emsp;▹ continuous random variable(연속확률변수): event 의 type 이 uncountable 한 경우<br>
&emsp;&emsp;▸ uncountable: uncountably infinite (real number)<br>
▸ probability mass function: for discrete X, input: x, output: P(X=x)<br>
&emsp;▹ $\sum_{x \in X} P(X=x) = 1$<br>
&emsp;▹ P(X=x) in (0,1)<br>
▸ probability density function: for continuous X, input: x, output: P(X=x)<br>
&emsp;▹ $\int_{x \in X} P(X=x) = 1$<br>
&emsp;▹ P(X=x) might be > 1 if size(event space/interval) < 1<br>
▸ hypothesis: 변수 or 확률변수의 값을 가정<br>
&emsp;▹ h=p<br>
▸ hypothesis testing: 가설이 진짜인지 검증 (?)<br>
▸ distribution(분포): 해당 확률변수의 모든 event와 그에 해당하는 확률값을 정의/요약<br>
&emsp;▹ pmf, pdf 와 동일<br>
▸ parameter: distribution 이 closed form 일 경우(?) 그 식을 결정하는 값들<br>
▸ expectation(기댓값):<br>
&emsp;▹ if X == variable, X 의 값을 이미 알고 있음<br>
&emsp;▹ if X == random variable, X 의 값을 모르고 있음<br>
&emsp;&emsp;▸ 이때 대신에 X 의 기댓값을 알수있음<br>
&emsp;&emsp;▸ X 가 가지는 모든 event x * event x 가 발생할 확률<br>
&emsp;▹ $E_{X\sim P} (X)$: random variable X 는 probability distribution P 를 따른다<br>
▸ mean: 해당 확률변수의 기댓값<br>
&emsp;▹ $E(X) = \sum_{x \in X}{x * P(X=x)}$<br>
▸ variance: 해당 확률변수가 얼마나 퍼져있는지<br>
&emsp;▹ $Var(X) = \sum_{x \in X}{(x-E(X))^2} * P(X=x)$<br>
...
&emsp;▹ 이미 발생한 과거의 사건들을 설명<br>
▸ bayesian probability: p = 특정 사건의 확실성 (level of certainty)<br>
&emsp;▹ 앞으로 발생할 미래의 사건들을 설명<br>
&emsp;▹ p=1 은 확실히 맞다, p=0 은 확실히 아니다</p>
