...
- KL divergence
    ...
    - D(P&#124;&#124;Q) = $E_{X\sim P}(\log \frac{P(X)}{Q(X)}) = \sum_{i=1}^{n}P(x_i) \log \frac{P(x_i)}{Q(x_i)}$
        ...
        - $\begin{aligned} D(P \vert \vert Q) &= \sum_{x=1}^{n} P(x_i)\log \frac{P(x_i)}{Q(x_i)}  \\ & =  \sum_{x=1}^{n} P(x_i)\log P(x_i) - \sum_{x=1}^{n} P(x_i)\log Q(x_i) \end{aligned}$
...
- Logistic Loss
    ...
    - $\begin{aligned}\text{Logistic loss}(f(X), Y) &= -(y \log f(x) + (1-y) \log (1-f(x))) \\& =\begin{cases} -log(f(x)) & \text{if $y=1$}\\ -log(1-f(x)) & \text{if $y=0$} \end{cases}\end{aligned}$
